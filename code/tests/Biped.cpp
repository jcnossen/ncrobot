...
std::vector<b2RevoluteJoint*> Biped::getJoints()
{
	b2RevoluteJoint *j[] = { LAnkle, RAnkle, LKnee, RKnee, LHip, RHip, 
		LowerAbs, UpperAbs, LowerNeck, UpperNeck,
		LShoulder, RShoulder, LElbow, RElbow, LWrist, RWrist };

	int n = sizeof(j)/sizeof(b2RevoluteJoint*);
	std::vector<b2RevoluteJoint*> joints (n);
	for(int i=0;i<n;i++)
		joints[i]=j[i];
